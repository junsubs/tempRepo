...
@WebServlet("/admin/souvenirinventory")
public class SouvenirInventoryController extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		try {
			...
			int cnt = as.souvenirInventoryCnt(searchType , searchValue);
			...
			//서비스
			List<SouvenirInventoryVo> voList = null;
			if(searchType == null || searchType.equals("")) {
				voList = as.souvenirInventory(pv);				
			}else {
				voList = as.souvenirInventory(pv, searchType, searchValue);				
			}
			...
			System.out.println("[ERR-303] 기념품재고조회 에러");
			e.printStackTrace();
		}
	}
