	...
	//결제취소화면
		...
		protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
			...
			try {
				...
				String sql = "SELECT PRICE FROM CAR_RESERVATION CR JOIN CAR_PAYMENT CP ON CR.NO = CP.CAR_RESERVATION_CODE WHERE CP.NO = ?";
		...
		protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
			CarPayCancelDao acdao = new CarPayCancelDao();
			...
			try {
				...
				result = acdao.carPaymentUpdate(conn, payNo);
			}catch(Exception e) {
				e.printStackTrace();
			}
			...
			int result2 = 0;
				try {
					...
					result2 = acdao.carPayCancelReason(conn2, pcvo);
