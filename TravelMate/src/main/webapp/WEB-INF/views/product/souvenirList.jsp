   ...
   <div id="page-area">
    	<c:if test="${pv.currentPage > 1}">
     	...
     	<c:forEach begin="${pv.startPage}" end="${pv.endPage}" step="1" var="i">
       	...
       	<a class="btn btn-primary btn-sm" href="${root}/souvenir/list?page=${i}&searchType=${searchVo.searchType}&searchValue=${searchVo.searchValue}&local=${searchVo.local}">${i}</a>
     		</c:if>
     		<c:if test="${pv.currentPage == i}">
     ...
     <c:if test="${pv.currentPage < pv.maxPage}">
     	<a class="btn btn-primary btn-sm" href="${root}/souvenir/list?page=${pv.currentPage + 1}&searchType=${searchVo.searchType}&searchValue=${searchVo.searchValue}&local=${searchVo.local}">다음</a>
     </c:if>
    </div>
...
function setSearchValueTagInput(){
	...
	tbody.addEventListener('click', (event)=>{
		...
		//요청보내기
		location.href='${root}/order/souvenir?name=' + name;
